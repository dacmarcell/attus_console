...
function App() {
  ...
  // ==========================================
  // INCIDENT DASHBOARD STATE & LOGIC
  // ==========================================
  ...
  const [messageInput, setMessageInput] = useState('Connection timeout on gateway')
  const [stackTraceInput, setStackTraceInput] = useState('java.net.SocketTimeoutException: Connect timed out')
  ...
  const handleSimulateIncident = async (type: 'TIMEOUT' | 'DATABASE' | 'MEMORY') => {
    ...
    if (type === 'TIMEOUT') {
      ...
      logPayload = {
        ...
        environment: 'production',
        level: 'FATAL',
        message: 'HikariPool-1 - Connection refused: Could not connect to database at postgres-primary:5432',
    ...
    try {
      ...
      alert(`Sucesso! 5 logs de ${type} foram ingeridos. Incidentes atualizados no Dashboard!`)
    } catch (err: any) {
  ...
  const handleSubmitProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.validationErrors ? Object.values(errorData.validationErrors).join(', ') : 'Erro ao processar produto.')
      }
  ...
  useEffect(() => {
    ...
    const handleGlobalError = (event: ErrorEvent) => {
      ...
      setErrorBannerMsg(`Capturado globalmente: "${msg}" (Log enviado com sucesso!)`)
      
      setTimeout(() => setErrorBannerMsg(null), 8000)
    }

    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      const reason = event.reason ? String(event.reason) : 'Promise rejeitada sem motivo'
      const stack = event.reason && event.reason.stack ? event.reason.stack : 'N/A'
      
      sendBrowserLog(`Unhandled Rejection: ${reason}`, stack)
      setErrorBannerMsg(`Capturado globalmente: Promise rejeitada - "${reason}" (Log enviado!)`)
  ...
  // Helper to send browser errors to /api/logs
  ...
  const renderDashboardView = () => (
    <div className="dashboard-grid">
      ...
      <section style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div className="panel">
          <h2 className="panel-title">
            <span className="nav-icon">⚡</span> Simulador de Incidentes
          </h2>
          <p className="panel-subtitle">
            Dispare rapidamente uma sequência de 5 logs idênticos à API para testar as regras de agrupamento de incidentes da V1.
          </p>
          ...
          <div className="simulator-buttons">
            ...
            >
              <div className="simulator-title">🌐 Exceção de Timeout</div>
              <div className="simulator-desc">Gera incidente NETWORK_TIMEOUT (HIGH)</div>
            </button>
            ...
            <button 
              ...
              disabled={isSendingSimulated}
            >
              <div className="simulator-title">💾 Falha de Banco de Dados</div>
              <div className="simulator-desc">Gera incidente DATABASE_FAILURE (CRITICAL)</div>
            </button>
            ...
            <button 
              ...
              disabled={isSendingSimulated}
            >
              <div className="simulator-title">🧠 Estouro de Memória Heap</div>
              <div className="simulator-desc">Gera incidente MEMORY_LEAK (CRITICAL)</div>
            </button>
        ...
        <div className="panel">
          <h2 className="panel-title">
            <span className="nav-icon">📥</span> Ingestão de Log
          </h2>
          <form onSubmit={handleManualSubmit}>
            ...
            <div className="form-group" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              ...
              <div>
                ...
                <button type="button" className="btn btn-secondary" onClick={fetchIncidents}>
                  🔄 Atualizar
                </button>
              </div>
            </div>
      ...
      <section>
        ...
        {/* API Error Status */}
        {dashboardError && (
          <div className="panel" style={{ borderColor: 'var(--severity-critical)', color: 'var(--severity-critical)', marginBottom: '20px' }}>
            <strong>⚠️ Conectividade API:</strong> {dashboardError}. Verifique se o backend está ativo na porta 8080.
          </div>
        )}
  ...
  const renderProductsView = () => (
    <div className="products-layout">
      ...
      <section>
        ...
        {productsError && (
          <div className="panel" style={{ borderColor: 'var(--color-danger)', color: 'var(--color-danger)', marginBottom: '20px' }}>
            <strong>⚠️ Erro de Rede:</strong> {productsError}
          </div>
        )}
      ...
      <section>
        <div className="panel">
          <h2 className="panel-title">
            <span className="nav-icon">{editingProduct ? '✏️' : '📥'}</span> 
      ...
      <div className="main-content">
        ...
        {errorBannerMsg && (
          <div className="error-banner">
            <span>⚠️ {errorBannerMsg}</span>
            <button className="error-banner-btn" onClick={() => setErrorBannerMsg(null)}>
              Fechar
            </button>
          </div>
        )}
        ...
        <div className="header-container">
          <h1>{activeView === 'dashboard' ? 'Dashboard de Incidentes' : 'Gestão de Produtos'}</h1>
          <p>
        ...
        {/* View Switch */}
        {activeView === 'dashboard' ? renderDashboardView() : renderProductsView()}
      </div>
